...
class ISRIStemmer():
    ...
    def __init__(self):
        ...
        # length one suffixes
        self.s1 = [u"\u0629", u"\u0647", u"\u064a", u"\u0643", u"\u062a", u"\u0627", u"\u0646"]
        ...
        self.re_short_vowels = re.compile(u"[\u064B-\u0652]")
        self.re_hamza = re.compile(u"[\u0621\u0624\u0626]")
        self.re_initial_hamza = re.compile(u"^[\u0622\u0623\u0625]")
    ...
    def norm(self, word, num=3):
        ...
        if num == 1:
            ...
            word = self.re_initial_hamza.sub(u"\u0627", word)
        elif num == 3:
            word = self.re_short_vowels.sub("", word)
            word = self.re_initial_hamza.sub(u"\u0627", word)
        return word
    ...
    def pro_w4(self, word):
        ...
        if word[0] in self.pr4[0]:  # مفعل
            ...
            word = word[:1] + word[2:]
        elif word[0] in self.pr4[4]:  # افعل - نفعل - تفعل - يفعل
            self.pre += word[0]
            word = word[1:]
            ...
            if len(word) <len(actual_word):
                self.suf=actual_word[-1]+self.suf
            if len(word) == 4:
                actual_word=word
                word = self.pre1(word)  # do - normalize short prefix
                if len(word)<len(actual_word):
                    self.pre=actual_word[0]
        return word,self.pre,self.suf

    def pro_w53(self, word):
        """process length five patterns and extract length three roots"""
        if word[0] == word[1] and word[0] == u"ت":  # تتفعل
            self.pre+=word[0:2]
            word = word[2:]

        elif word[2] in self.pr53[0] and word[0] == u"\u0627":  # افتعل - افاعل
            self.pre+=word[0]
            word = word[1] + word[3:]

        elif word[3] in self.pr53[1] and word[0] == u"\u0645":  # مفعول - مفعال - مفعيل
            self.pre+=word[0]
            word = word[1:3] + word[4]

        elif word[1] == u"ا" and word[4] == u"ة": #  فاعلة
            self.suf=word[-1]+self.suf
            word = word[0] + word[2] + word[3]

        elif word[0] in self.pr53[2] and word[4] == u"\u0629":  # مفعلة - تفعلة - افعلة
            self.pre+=word[0]
            self.suf = word[-1] + self.suf
            word = word[1:4]

        elif word[0] in self.pr53[3] and word[2] == u"\u062a":  # مفتعل - يفتعل - تفتعل
            self.pre+=word[0]
            word = word[1] + word[3:]

        elif word[0] in self.pr53[4] and word[2] == u"\u0627":  # مفاعل - تفاعل
            self.pre+=word[0]
            word = word[1] + word[3:]

        elif word[2] in self.pr53[5] and word[4] == u"\u0629":  # فعولة - فعالة
            self.suf = word[-1] + self.suf
            word = word[:2] + word[3]

        elif word[0] in self.pr53[6] and word[1] == u"\u0646":  # انفعل - منفعل
            self.pre+=word[0]
            word = word[2:]

        elif word[3] == u"\u0627" and word[0] == u"\u0627":  # افعال
            self.pre+=word[0]
            word = word[1:3] + word[4]

        elif word[4] == u"\u0646" and word[3] == u"\u0627":  # فعلان
            self.suf = word[-2] + self.suf
            word = word[:3]

        elif word[3] == u"\u064a" and word[0] == u"\u062a":  # تفعيل
            self.pre+=word[0]
            word = word[1:3] + word[4]

        elif word[3] == u"\u0648" and word[1] == u"\u0627":  # فاعول
            word = word[0] + word[2] + word[4]
        elif word[2] == u"\u0627" and word[1] == u"\u0648":  # فواعل
            word = word[0] + word[3:]
        elif word[3] == u"\u0626" and word[2] == u"\u0627":  # فعائل
            word = word[:2] + word[4]

        elif word[4] == u"\u0629" and word[1] == u"\u0627":  # فاعلة
            self.suf = word[-1] + self.suf
            word = word[0] + word[2:4]

        elif word[4] == u"\u064a" and word[2] == u"\u0627":  # فعالي
            self.suf = word[-1] + self.suf
            word = word[:2] + word[3]
        else:
            actual_word=word
    ...
    def pro_w54(self, word):
        ...
        elif word[4] == u"\u0629":  # فعللة
            self.suf = word[-1] + self.suf
            word = word[:4]

        elif word[2] == u"\u0627":  # فعالل
            #self.suf = word[-1] + self.suf
            word = word[:2] + word[3:]
        return word,self.pre,self.suf
    ...
    def end_w5(self, word):
        ...
        if len(word) == 4:
            word,self.pre,self.suf = self.pro_w4(word)
        elif len(word) == 5:
            word,self.pre,self.suf = self.pro_w54(word)
        return word,self.pre,self.suf

    def pro_w6(self, word):
        """process length six patterns and extract length three roots"""
        if word.startswith(u"\u0627\u0633\u062a") or word.startswith(
            u"\u0645\u0633\u062a"
        ):  # مستفعل - استفعل
            self.pre+=word[0:3]
            word = word[3:]
            ...
            word[0] == u"م" and word[1] == u"ت" and word[3] == u"ا"
        ):  # متفاعل - متفاعلة
            self.pre+=word[0:2]
            word = word[2] + word[4:]

        elif (
            word[0] == u"\u0645" and word[3] == u"\u0627" and word[5] == u"\u0629"
        ):  # مفعالة
            self.pre+=word[0]
            self.suf=word[-1]+self.suf
            word = word[1:3] + word[4]
            ...
            word[0] == u"\u0627" and word[2] == u"\u062a" and word[4] == u"\u0627"
        ):  # افتعال
            self.pre+=word[0]
            word = word[1] + word[3] + word[5]

        elif (
            word[0] == u"\u0627" and word[3] == u"\u0648" and word[2] == word[4]
        ):  # افعوعل
            self.pre+=word[0]
            word = word[1] + word[4:]

        elif (
            word[0] == u"\u062a" and word[2] == u"\u0627" and word[4] == u"\u064a"
        ):  # تفاعيل   new pattern
            self.pre+=word[0]
            word = word[1] + word[3] + word[5]

        elif (
            word[0] == u"م" and word[2] == u"ا" and word[4] == u"ي"
        ):  # مفاعيل
            self.pre+=word[0]
            word = word[1] + word[3] + word[5]
    ...
    def pro_w64(self, word):
        ...
        if word[0] == u"\u0627" and word[4] == u"\u0627":  # افعلال
            self.pre+=word[0]
            word = word[1:4] + word[5]

        elif word.startswith(u"\u0645\u062a"):  # متفعلل
            self.pre+=word[0:2]
            word = word[2:]
        return word,self.pre,self.suf
    ...
    def end_w6(self, word):
        ...
        if len(word) == 5:
            word,self.pre,self.suf = self.pro_w53(word)
            word,self.pre,self.suf = self.end_w5(word)
        elif len(word) == 6:
            word,self.pre,self.suf = self.pro_w64(word)
        return word,self.pre,self.suf
